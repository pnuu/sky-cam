...
static int writeImage(char *filename, struct frame img_in, char *title) {
    ...
    img_out =
        (unsigned char *)malloc(n * img_in.channels * sizeof(unsigned char));
    ...
    if (img_in.channels == 3) {
        ...
        for (i = 0, j = 0, k = 0; i < n; i += 2, j += 6, k++) {
            ...
            img_out[j + 1] =
                limit(Y[i] + 0.3455 * (Cb[k] - 128) - (0.7169 * (Cr[k] - 128)));
    ...
    switch (img_in.channels) {
        case 3:
            ...
            png_set_IHDR(png_ptr, info_ptr, img_in.width, img_in.height, 8,
    ...
    row = (png_bytep)malloc(img_in.channels * img_in.width * sizeof(png_byte));
...
static int save_data_to_png(char *filename, struct data_image *data_in,
    ...
    */
    ...
    /* Band-interleaved RGB image */
    png_set_IHDR(png_ptr, info_ptr, data_in->width, data_in->height, 8,
                 PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE,
                 PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
...
void *save_stacks(void *ptr) {
    ...
    if (stacks->latest != NULL) {
        printf("Saving %s\n", stacks->latest_fname);
        writeImage(stacks->latest_fname, *(stacks->latest),
                   "Finsprite latest stack");
        for (i = n; i--;) {
            stacks->latest->Y[i] = 0;
        }
        /*
    ...
    if (stacks->ave24 != NULL) {
        ...
        if (stacks->ave24->Cr != NULL) {
            for (i = n / 2; i--;) {
                stacks->ave24->Cr[i] = stacks->ave24->Cb[i] = 0;
            }
        }
    }
    ...
    if (stacks->ave8 != NULL) {
        ...
        if (stacks->ave8->Cr != NULL) {
            for (i = n / 2; i--;) {
                stacks->ave8->Cr[i] = stacks->ave8->Cb[i] = 0;
            }
        }
    }
    ...
    if (stacks->max != NULL) {
        ...
        printf("Saving %s, %d frames used.\n", fname, stacks->num_imgs);
        // stretch(peak_hold->img_buffer, peak_hold->width*peak_hold->height,
        // 0.01, 0.99);
    ...
    if (stacks->times != NULL) {
        ...
        for (i = n; i > 2; i--) {
            if (stacks->last_times->Y[i] != 0) {
                time_temp =
                    (stacks->last_times->Y[i] + stacks->times->Y[i]) >> 1;
    ...
    if (stacks->min != NULL) {
        ...
        printf("Saving %s, %d frames used.\n", fname, stacks->num_imgs);
        // stretch(min_hold->img_buffer, min_hold->width*min_hold->height, 0.01,
        // 0.99);
        /* Save image    */
...
static void write_log(struct stacks *stacks) {
    ...
    fprintf(log_fid, "%s,%d,%d,%d,%d,%d,%d,%d,%d\n", tmp, stacks->length,
            stacks->num_imgs, saved_stacks[0], saved_stacks[1], saved_stacks[2],
            saved_stacks[3], saved_stacks[4], saved_stacks[5]);
    fclose(log_fid);
...
static char *make_time_string(struct stacks *stacks, int type) {
    ...
    switch (type) {
        ...
        case STACK_LOG_DATA:
            ...
            strftime(time_string, 30, "%Y,%m,%d,%H,%M,%S",
                     gmtime(&(timestamp)));
...
static void save_ave_stack(struct stacks *stacks) {
    ...
    if (channels == 3) {
        out_frame.Cr =
            (unsigned char *)malloc(width * height * sizeof(unsigned char) / 2);
        out_frame.Cb =
            (unsigned char *)malloc(width * height * sizeof(unsigned char) / 2);
    }
    ...
    if (channels == 3) {
        ...
        // out_frame[i] =
        // 255*ave_stack_save->img_buffer[i]/ave_stack_save->max_val;
        // out_frame[i] =
        // 255*(ave_stack_save->img_buffer[i]-ave_stack_save->min_val)/(ave_stack_save->max_val-ave_stack_save->min_val);
    }

    make_fname(fname, stacks, AVE8);
    // stretch(out_frame, ave_stack_save->width*ave_stack_save->height, 0.02,
    // 0.98);
    printf("Saving %s, %d frames used.\n", fname, num_img);
