...
void init_current_frame(struct frame *current_frame, int width, int height,
                        int channels) {
...
void init_peak_hold(struct frame *peak_hold, int width, int height,
                    int channels) {
    peak_hold->Y = calloc(width * height, sizeof(unsigned char));
    if (channels == 3) {
        peak_hold->Cr = calloc(0.5 * width * height, sizeof(unsigned char));
        peak_hold->Cb = calloc(0.5 * width * height, sizeof(unsigned char));
    }
...
void init_min_hold(struct frame *min_hold, int width, int height,
                   int channels) {
    int i = 0;
    min_hold->Y = malloc(width * height * sizeof(unsigned char));
    for (i = 0; i < width * height; i++) {
        min_hold->Y[i] = 255;
    }
    if (channels == 3) {
        min_hold->Cr = malloc(0.5 * width * height * sizeof(unsigned char));
        min_hold->Cb = malloc(0.5 * width * height * sizeof(unsigned char));
    }
...
void init_latest_img(struct frame *latest_img, int width, int height,
                     int channels) {
    latest_img->Y = calloc(width * height, sizeof(unsigned char));
    if (channels == 3) {
        latest_img->Cr = calloc(0.5 * width * height, sizeof(unsigned char));
        latest_img->Cb = calloc(0.5 * width * height, sizeof(unsigned char));
    }
...
void init_ave_stack(struct data_image *ave_stack, int width, int height,
                    int channels) {
    ave_stack->Y = calloc(width * height, sizeof(unsigned long));
    if (channels == 3) {
        ave_stack->Cr = calloc(0.5 * width * height, sizeof(unsigned long));
        ave_stack->Cb = calloc(0.5 * width * height, sizeof(unsigned long));
    }
...
void init_mmap(const int fd, struct buffer *mmap_buffers) {
    ...
    */
    ...
    for (i = 0; i < req.count; ++i) {
        ...
        if (MAP_FAILED == mmap_buffers[i].start) errno_exit("mmap");
    }
}
...
void init_device(const int fd, const char *dev_name, struct v4l2_format *fmt) {
    ...
    if (xioctl(fd, VIDIOC_S_FMT, fmt) == -1) errno_exit("VIDIOC_S_FMT");
